...
global xBot %added to allow receiver placement along bty
global Pos
global alpha Nbeams
global U Arr MxNarr
...
%     if NumDepths==1
%         Pos.r.range=0;
%         Pos.r.depth=Pos.s.depth+0.5;
%     end
    %if n==NumDepths
    %   Pos.r.range=rvals(n)+abs(rvals(n)-rvals(n-1)) % KP Interp
    %end
    %else
    %   Pos.r.range=(rvals(n+1)-rvals(n)) % for known path
    %end
    ...
    readbty_drift( btyfil, BotBTY, Bdry.Bot.depth, Beam.Box.r, -1*rvals(n) );    % read in the bathymetry file
    % Used to set receiver position as equal to bty points
    Pos.r.range = xBot(1,:);
    Pos.r.depth = xBot(2,:);
    ...
    if Beam.RunType(1:1) == 'R'
        figure(n)
        clf;
        plotbty_drift(filename, -1*rvals(n)); % negative = source moves right
    end
    for ibeam = 1 : Nbeams
    ...
    if ( Beam.RunType(1:1) == 'A' )   % arrivals calculation
        ...
        save( strcat('Pos',num2str(n),'-d',num2str(dvals(n)),'-r',num2str(rvals(n)),'-',...
            arrfil), 'PlotTitle', 'Pos', 'Arr', 'MxNarr' )
    else
        ...
        save( strcat('Pos',num2str(n),'-d',num2str(dvals(n)),'-r',num2str(rvals(n)),'-',...
            shdfil),'PlotTitle', 'PlotType', 'freq', 'atten', 'Pos', 'pressure')
        %plotshd( shdfil )
    end
