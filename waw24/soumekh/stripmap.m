...
f0=250;                 % baseband bandwidth is 2*f0
w0=pi2*f0;
...
%
Xc=250;                  % Range distance to center of target area
X0=100;                  % Target area in range is within [Xc-X0,Xc+X0]
Y0=120;                  % Target area in cross-range is within [-Y0,Y0]
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    u domain parameters and arrays for SAS signal     %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
du=D/1; % changed from 4 to 1     % sample spacing in aperture domain
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% u domain parameters and arrays for compressed SAS signal %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
Ik=find(k >= kmin & k <= kmax); % Indices of relevant wave numbers within frequency range - will
[temp,nk]=size(Ik);
lambda=zeros(1,n);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
ntarget=1000;                        % number of targets
...
% Single Layer Bathymetry
for nloop=1:ntarget
    xn(nloop)= nloop;
    yn(nloop)= -95+5*sin(1/5*xn(nloop));
    % yn(nloop)=-100;
    fn(nloop)=1.;
end

% Multi Layer Bathymetry
% for nloop=1:1000
%     xn(nloop)= (nloop-1)/2;
%     xn(nloop+1000) = (nloop - 1)/2;
%     yn(nloop)= -95+5*sin(1/5*xn(nloop));
%     yn(nloop+1000)= -96+5*sin(1/5*xn(nloop));
%     % yn(nloop)=-100;
%     fn(nloop)=1.;
%     fn(nloop+1000)=1.;
% end

% Multi-layer bathymetry v2
% for mloop=1:20
%     for nloop=1:50
%         xn(nloop+(mloop-1)*50)= nloop*20;
%         yn(nloop+(mloop-1)*50)= -95+5*sin(1/5*xn(nloop))-mloop*0.5;
%         fn(nloop+(mloop-1)*50)=1.0;
%     end
% end
...
%
parfor i=1:ntarget;
 td=t(:)*ones(1,m)-2*ones(n,1)*sqrt((Xc+xn(i))^2+(yn(i)-u).^2)/c; % time delay? - will
 sn=fn(i)*exp(cj*wcm*td+cj*alpha*(td.^2)).*(td >= 0 & td <= Tp & ...
   ones(n,1)*abs(u) <= L & t(:)*ones(1,m) < Tf); 
% wcm = modified chirp carrier, alpha = chirp rate. This line appears to be
% the calculation of the echo signal in the fast-time domain based on the
% provided reflector points - will

 sn=sn.*exp(-cj*wc*t(:)*ones(1,m));      % Fast-time baseband conversion

%
% calculate (omega,u)-domain beam pattern
     theta_n=ones(nk,1)*atan((yn(i)-u)/(Xc+xn(i))); % bearing of nth-point to receiver
     beam_p=zeros(n,m); % beam-pattern
     beam_p(Ik,:)=(.5+.5*cos((pi*theta_n)./(phi_d(Ik).'*ones(1,m)))).* ...
        (abs(theta_n) <= phi_d(Ik).'*ones(1,m)); 
        % Note: Ik is a vector of indicies, phi_d is divergence angle - will
     
% incorporate beam pattern in (omega,u) domain
%
  s=s+iftx(ftx(sn).*beam_p); % ftx = forward fft w.r.t. first variable, iftx is inverse
end;
%

G=abs(s)'; % Still not absolutely sure what G stands for if anything - will
xg=max(max(G)); ng=min(min(G)); cg=255/(xg-ng); % double max/min used bc matrix
figure(1) % 
image(t,u,256-cg*(G-ng));
axis('square');axis('xy')
xlabel('Fast-time t, sec')
ylabel('Synthetic Aperture (Slow-time) U, meters')
title('Measured Stripmap SAS Signal')
pause(1)
%
td0=t(:)-2*Xc/c; % time-delay for center of focus region? - will
...
% (Note: conj() returns complex conjugate)
%

G=abs(iftx(s))';
xg=max(max(G)); ng=min(min(G)); cg=255/(xg-ng);
tm=(2*Xc/c)+dt*(-n/2:n/2-1);   % fast-time array after matched filtering
figure(2) % Based on the appearance of the related graph, filters out most
    % of the signal on the u-axis leaving the strongest that occur close to
    % the bathymetry depth (-100 m) - will
image(tm,u,256-cg*(G-ng));
axis('square');axis('xy')
xlabel('Fast-time t, sec')
ylabel('Synthetic Aperture (Slow-time) U, meters')
title('Stripmap SAS Signal after Fast-time Matched Filtering')
pause(1)
...
% Note from Will - Soumekh doesn't really explain how to do this conversion
% very well, neither in the readme nor the section of the textbook
% describing the code. Will update if I figure out how. 
%
% Upsample data before compression for digital spotlighting
%
fs=fty(s); % fty = forward fft based on second variable
...
s_ds=zeros(n,mc);           % Initialize digital spotlight SAS array
...
for i=1:ns; i                 % loop for each subaperture
    ...
    xg=max(max(G)); ng=min(min(G)); cg=255/(xg-ng);
    figure(3) % Currently seems to be basically blank, all yellow - Will
    image(tm,kuc,256-cg*(G-ng));
    xlabel('Fast-time t, sec')
    ylabel('Synthetic Aperture (Slow-time) Doppler Frequency k_u, rad/m')
    title('Polar Format SAS Reconstruction with Digital Spotlight Filter')
    axis square; axis xy;
    pause(1)
...
%
fs=fty(s_ds);              % F.T. of SAS signal w.r.t. u
%
...
% xlabel('Fast-time t, sec')
% ylabel('Synthetic Aperture (Slow-time) U, meters')
% title('Stripmap SAS Signal after Digital Spotlighting')
% pause(1)
% 
% G=abs(fs)';
% xg=max(max(G)); ng=min(min(G)); cg=255/(xg-ng);
...
% title('Stripmap SAS Signal Spectrum after Digital Spotlighting')
% pause(1)
...
if Y0 < L,
 ...
 fs=fs(:,I);                % subsampled SAS signal spectrum
...
title('Stripmap SAS Spatial Frequency Data Coverage')
axis image; axis xy
pause(1)
%
kxmin=min(min(kx));
...
xlabel('Spatial Frequency k_x, rad/m')
ylabel('Spatial Frequency k_y, rad/m')
title('Wavefront Stripmap SAS Reconstruction Spectrum')
pause(1)
...
%
% Display SAS reconstructed image
...
title('Wavefront Stripmap SAS Reconstruction')
pause(1)
...
parfor i=1:nx; i        % Stack's loop for reconstruction at each range
...
xlabel('Range X, meters')
ylabel('Cross-range Y, meters')
title('Range Stack Stripmap SAS Reconstruction')
pause(1)
...
xlabel('Spatial Frequency k_x, rad/m')
ylabel('Spatial Frequency k_y, rad/m')
title('Range Stack Stripmap SAS Reconstruction Spectrum')
pause(1)
% 
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%          (Iu & ones(n,1)*(t_ij >= Ts & t_ij <= Tf)))); % removed . after s_ds
%    end;
% end;
% 
% % Remove carrier in range domain
...
% xlabel('Range X, meters')
% ylabel('Cross-range Y, meters')
% title('TDC Stripmap SAS Reconstruction')
% pause(1)
%                 
...
% xlabel('Spatial Frequency k_x, rad/m')
% ylabel('Spatial Frequency k_y, rad/m')
% title('TDC Stripmap SAS Reconstruction Spectrum')
% pause(1)
...
xlabel('Range X, meters')
ylabel('Cross-range Y, meters')
title('Backprojection Stripmap SAS Reconstruction')
pause(1)
...
xlabel('Spatial Frequency k_x, rad/m')
ylabel('Spatial Frequency k_y, rad/m')
title('Backprojection Stripmap SAS Reconstruction Spectrum')
pause(1)
