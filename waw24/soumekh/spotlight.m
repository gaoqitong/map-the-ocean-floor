...
Y0=50;                  % target area in cross-range is within
                         % [Yc-Y0,Yc+Y0]

% Case 1: L < Y0; requires zero-padding of SAR signal in synthetic
% aperture domain
%
  L=50;                 % synthetic aperture is 2*L
...
%
dt=1/(1*f0);%changed 4 to 1  % Time domain sampling (guard band factor 2)
...
for nloop=1:ntarget
    xn(nloop)= nloop-1;
    yn(nloop)= -100+5*sin(1/5*xn(nloop));
    fn(nloop)=1.;
end
...
%
parfor i=1:ntarget;   % Loop for each target
...
% print P5.1.ps % commented out all % prints because throw error
pause(1)
%
