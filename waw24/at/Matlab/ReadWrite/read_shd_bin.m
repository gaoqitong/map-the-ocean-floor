function [ title, PlotType, freq, atten, Pos, pressure ] = read_shd_bin( varargin )
...
if nargin == 2
    % Generate a warning.
    Message = 'Call read_shd_bin with 1 or 3 inputs.';
    warning( Message );
end
...
if ( fid == -1 )
    error( 'No shade file with that name exists; you must run a model first', 'read_shd_bin' );
end
...
if isnan( xs )    % Just read the first sd, all rd
    
    for itheta = 1 : Ntheta
        for isd = 1 : Nsd
            % disp( [ 'Reading data for source at depth ' num2str( isd ) ' of ' num2str( Nsd ) ] )
            for ird = 1 : Nrcvrs_per_range
                recnum = 9 + ( itheta - 1 ) * Nsd * Nrcvrs_per_range + ( isd - 1 ) * Nrcvrs_per_range + ird - 1;
    ...
    for itheta = 1 : Ntheta
        for isd = 1 : Nsd
            % disp( [ 'Reading data for source at depth ' num2str( isd ) ' of ' num2str( Nsd ) ] )
            for ird = 1 : Nrcvrs_per_range
                recnum = 9 + ( idxX   - 1 ) * Nsy * Ntheta * Nsd * Nrcvrs_per_range + ...
