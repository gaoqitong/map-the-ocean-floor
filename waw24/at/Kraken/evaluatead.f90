SUBROUTINE EvaluateAD( FileRoot, rProf, NProf, phiS, phiR, rd, Nrd, r, Nr, M, Option, P )
  ...
  REAL                  :: freq, rLeft, rMid, w, wMid
  COMPLEX               :: phiL( MaxM, Nrd ), &
                           SUM, kInt( MaxM ), phiINT( MaxM ), kL( MaxM ), kR( MaxM ), kMid( MaxM )
  ...
  sumk(    1 : M ) = 0.0
  sumkinv( 1 : M ) = 0.0
  IF ( Option( 1 : 1 ) == 'T' ) const( 1 : M ) = const( 1 : M ) / SQRT( kL( 1 : M ) )
...
SUBROUTINE NewSegment( FileRoot, r, ir, rProf, iProf, NProf, kL, kR, phiL, phiR, M, MaxM, rd, Nrd, sumk, sumkinv )
  ...
  IF ( iProf + 1  <= NProf ) THEN   ! is there a new mode set to read?
     CALL GetModes( FileRoot, iProf + 1, MaxM, rd, Nrd, 'N', kR, phiR, MR, freq, Title )
     M = MIN( M, MR )
