PROGRAM BELLHOP
  ...
  IF ( Inline ) THEN
     ...
     Bdry%Bot%HS%Opt   = 'A_'
     Bdry%Bot%HS%BC    = 'A'
     Bdry%Bot%HS%cp    = CRCI( 1590D0,    0.5D0, freq, AttenUnit )   ! compressional wave speed
     Bdry%Bot%HS%cs    = CRCI(    0D0,      0D0, freq, AttenUnit )   ! shear         wave speed
...
SUBROUTINE BellhopCore
  ...
  IF ( atiType( 2 : 2 ) == 'L' ) THEN
     DO iSeg = 1, NatiPts
        Top( iSeg )%HS%cp = CRCI( Top( iSeg )%HS%alphaR, Top( iSeg )%HS%alphaI, freq, 'W ' )   ! compressional wave speed
        Top( iSeg )%HS%cs = CRCI( Top( iSeg )%HS%betaR,  Top( iSeg )%HS%betaI,  freq, 'W ' )   ! shear         wave speed
     END DO
  END IF

  IF ( btyType( 2 : 2 ) == 'L' ) THEN
     DO iSeg = 1, NbtyPts
        Bot( iSeg )%HS%cp = CRCI( Bot( iSeg )%HS%alphaR, Bot( iSeg )%HS%alphaI, freq, 'W ' )   ! compressional wave speed
        Bot( iSeg )%HS%cs = CRCI( Bot( iSeg )%HS%betaR,  Bot( iSeg )%HS%betaI,  freq, 'W ' )   ! shear         wave speed
     END DO
  END IF
