SUBROUTINE READIN( FileRoot, ThreeD )
  ...
  IF ( Bdry%Top%HS%BC == 'A' ) WRITE( PRTFile, FMT = "(//, 'Z    alphaR     betaR      rho     alphaI     betaI', /)" )
...
SUBROUTINE TopBot( ENVFile, PRTFile, freq, AttenUnit, HS )
  ...
  CASE ( 'G' )                  ! *** Grain size (formulas from UW-APL HF Handbook)
     ...
     HS%cp  = CRCI( alphaR, alphaI, freq, 'L' )
     HS%cs  = 0.0
     HS%rho = rhoR
...
SUBROUTINE OpenOutputFiles( FileRoot, ThreeD )
  ...
  CASE ( 'A' )        ! arrival file in ascii format
     ...
     WRITE( ARRFile, * ) freq, Pos%Nsd, Pos%Nrd, Pos%Nr, Pos%Ntheta
     WRITE( ARRFile, * ) Pos%sd( 1 : Pos%Nsd )
     WRITE( ARRFile, * ) Pos%rd( 1 : Pos%Nrd )
     WRITE( ARRFile, * ) Pos%r(  1 : Pos%NR  )
     WRITE( ARRFile, * ) Pos%theta( 1 : Pos%Ntheta)
  ...
  CASE DEFAULT
     ...
     CALL WriteHeader( TRIM( FileRoot ) // '.shd', Title, SNGL( freq ), atten, PlotType )
  END SELECT
