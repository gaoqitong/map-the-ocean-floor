...
SUBROUTINE GETPAR( FileRoot )
  ...
  Title = 'SPARC-   '
  CALL READIN( FileRoot, Title, freq, MaxMedium, TopOpt, HSTopD, N, BotOpt, HSBotD, ENVFile, PRTFile )
  ...
  CASE ( 'S' )   ! snapshot
     ...
     DO Itout = 1, Ntout
        RcvrDepth: DO ir = 1, Pos%Nrd
           ...
           WRITE( GRNFile, REC = 9 + IG) Green( Itout, ir, 1:Nk )
        END DO RcvrDepth
     END DO
     CLOSE( GRNFile )
  CASE ( 'D' )   ! RTS (vertical array)
     Scale = 1.0 / SQRT( SNGL( pi ) * Pos%r( 1 ) )
     DO Itout = 1, Ntout
...
SUBROUTINE WriteHeaderSparc( FileRoot )
  ...
  REAL               :: Atten = 0, freq = 0  ! no stabilizing attenuation in a Sparc run
